...
def trim_description_text(desc_warning):
    ...
    hidden_symbol = "..."
    limit_len = 999
    text_trimmed = trim_block_text(desc_warning,
                                   limit_len,
                                   len_preserv_right,
                                   hidden_symbol)
    return text_trimmed
...
def trim_block_text(text_content,
                    limit_len,
                    len_preserv_right=15,
                    hidden_symbol='...'):
    ...
    start_max_len = max([len(line_content) for line_content in list_line_content])
    ...
    while True:
        for index, newlines in enumerate(list_newlines):
            if len(newlines) > max_len_allowed:
                line_trim = trim_string(newlines,
                                        1,
                                        len_preserv_right,
                                        hidden_symbol)
